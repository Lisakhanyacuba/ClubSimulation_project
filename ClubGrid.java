...
public class ClubGrid {
	...
	ClubGrid(int x, int y, int [] exitBlocks,PeopleCounter c) throws InterruptedException {
      ...
      initialLoc = Blocks[getMaxX()/2][bar_y+1];
		counter=c;
		}
 ...
 entrance.notifyAll();
   ...
   { 
     ...
     entrance.wait();
		 ...
		 //add to counter of people waiting 
		entrance.get(myLocation.getID());
		counter.personEntered(); //add to counter
      //They set the location of the entrance , so ? set yours using this yepyy
		myLocation.setLocation(entrance);
		myLocation.setInRoom(true);
		return entrance; 
      }
      }
   public GridBlock BarmanLocation(PeopleLocation myLocation)
   
   {
     myLocation.setLocation(initialLoc);
     myLocation.setInRoom(true);
     return initialLoc ;
   }
...
//====================================================================
// Move for the Barman
	public GridBlock move(GridBlock currentBlock,int step_x,PeopleLocation myLocation) throws InterruptedException {  //try to move in 
		
		int c_x= currentBlock.getX();
		int c_y= currentBlock.getY();
		
		int new_x = c_x+step_x; //new block x coordinates
		int new_y = currentBlock.getY() ; // new block y  coordinates
...
//Use it for barman to locate its position.
