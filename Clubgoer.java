...
public class Clubgoer extends Thread {
	//Synchronize everything that has to do with this ClubGrid shared Class.
	public static ClubGrid club; //shared club
   public static AtomicBoolean inBar = new AtomicBoolean(false);
...
public static ArrayList<GridBlock> positions = new ArrayList<GridBlock>() ;
	...
	public void checkPause() {
		// THIS DOES NOTHING - MUST BE FIXED  	
         while (ClubSimulation.paused.get()==true) {
            try {
	...
	//get drink at bar
		public void getDrink() throws InterruptedException {
         ...
         while(Barman.served.get()==false)
         {
         currentBlock.wait();
         }
         System.out.println("I realised current thread"+this.ID);
         }
			thirsty=false;
	...
	//leave club
	synchronized private void leave() throws InterruptedException {
	...
	public void run() {
		try {
			...
			checkPause();
         // Set yours to the Known location, what ? the Barman yeah.
			...
			// This is enter the club take note
         enterClub();
			...
			while (inRoom) {	
				...
				if (wantToLeave) {	 //leaving overides thirsty	
                  ...
                  inBar.set(true);
						...
						getDrink();
                  // This will provide you the ID and the rest of the details of the Thread
